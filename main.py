...
class SpriteSomethingMainFrame(tk.Frame):
	...
	def time_marches_forward(self):
		...
		if not self.freeze_ray:  #freeze rays impress the ladies, according to Dr. Horrible
			self.advance_global_frame_timer()
			end_time = time.perf_counter()
			lag = (end_time-start_time)*1000
			wait_time = int(FRAME_DELAY/self._current_speed - lag)
	...
	def create_menu_bar(self, game_name):
		...
		export_menu.add_command(label="Copy to new Game File", image=export_menu.images["export-sfc-new"], compound=tk.LEFT, command=self.copy_to_game_file)

		img = tk.PhotoImage(file=os.path.join("resources","meta","icons","inject.png"))
		export_menu.images["export-sfc"] = img
		export_menu.add_command(label="Inject into Game File", image=export_menu.images["export-sfc"], compound=tk.LEFT, command=self.inject_into_game_file)
	...
	def copy_to_game_file(self):
		old_rom_filename = filedialog.askopenfilename(initialdir="./", title="Select Game File to Copy", filetypes=(("SNES ROMs","*.sfc;*.smc"),))
		new_rom_filename = filedialog.asksaveasfilename(defaultextension=".sfc", initialdir="./", title="Save Copy of Game To", filetypes=(("SNES ROMs","*.sfc;*.smc"),))
		if old_rom_filename and new_rom_filename:
			try:
				new_rom = self.sprite.export_to_ROM(old_rom_filename)
				new_rom.save(new_rom_filename,overwrite=True)
				tk.messagebox.showinfo("Copy success",f"Exported sprite to {new_rom_filename}")
			except AssertionError:
				tk.messagebox.showerror("Copy error",f"ERROR: Cannot save {new_rom_filename}")
	def inject_into_game_file(self):
		rom_filename = filedialog.asksaveasfilename(initialdir="./", title="Select Game File to Modify", filetypes=(("SNES ROMs","*.sfc;*.smc"),))
		if rom_filename:
			try:
				new_rom = self.sprite.export_to_ROM(rom_filename)
				new_rom.save(rom_filename,overwrite=True)
				tk.messagebox.showinfo("Export success",f"Exported sprite to {rom_filename}")
			except AssertionError:
				tk.messagebox.showerror("Export error",f"ERROR: Cannot save {rom_filename}")
	...
	def step_global_pose_timer(self):
		...
		starting_pose = self.sprite.get_pose_number_from_frame_number(animation_ID, self._frame_number)
		for _ in range(100): #100 is a failsafe here
			self._frame_number += 1
			if starting_pose != self.sprite.get_pose_number_from_frame_number(animation_ID, self._frame_number):
				self.update_sprite_animation()
				return
	...
	def rewind_global_pose_timer(self):
		...
		for _ in range(100): #100 is a failsafe here
			self._frame_number -= 1
			if self._frame_number <= 0 or starting_pose != self.sprite.get_pose_number_from_frame_number(animation_ID, self._frame_number):
				self.update_sprite_animation()
				return
